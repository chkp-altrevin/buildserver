...
# ---------------- Windows Host Environment Fallback ----------------

detect_unsupported_windows_terminal() {
  if [[ "$OS" == "Windows_NT" || "$(uname -o 2>/dev/null)" == "Msys" ]]; then
    if ! grep -qi microsoft /proc/version 2>/dev/null && ! [[ -d "/mnt/c" ]]; then
      echo "[WARN] Detected a native Windows terminal without Git Bash or WSL2."
      echo "[INFO] This script is not designed to run natively in Windows Command Prompt or PowerShell."
      echo ""
      echo "To continue, you can:"
      echo "  1. Install Git Bash: https://git-scm.com/downloads"
      echo "  2. Install VirtualBox: https://www.virtualbox.org/wiki/Downloads"
      echo "  3. Install Vagrant: https://developer.hashicorp.com/vagrant/downloads"
      echo ""
      echo "After installing, run this script from Git Bash or WSL."
      echo ""
      read -rp "Exit now and install manually? [Y/n]: " user_exit
      case "$user_exit" in
        [nN]*) return 0 ;;
        *) echo "Exiting."; exit 1 ;;
      esac
...
handle_vagrantfile_missing() {
  echo "[INFO] Vagrantfile not found in $PROJECT_PATH."
  echo "[INFO] Attempting to download required repo using: ./install-script.sh --repo-download"
  if ./install-script.sh --repo-download; then
    if [[ -f "$PROJECT_PATH/Vagrantfile" ]]; then
      echo "[INFO] Vagrantfile downloaded successfully."
      cd "$PROJECT_PATH" && vagrant up
    else
      echo "[ERROR] Vagrantfile still not found after download."
      echo "[ERROR] Current path: $(pwd)"
      echo "Please ensure a valid Vagrantfile exists in $PROJECT_PATH and re-run the script."
      exit 1
    fi
  else
    echo "[ERROR] Failed to download repository. Please check your connection or download manually."
    exit 1
  fi
}
