...
handle_virtualbox_vagrant_win() {
  log_info "Detected --virtualbox-vagrant-win flag"

  if [[ "$OS" != "Windows_NT" && "$(uname -o 2>/dev/null)" != "Msys" ]]; then
    log_info "You must run this script using Git Bash or WSL on Windows."
    read -rp "Install Git Bash now? (yes/no): " confirm_gitbash
    case "$confirm_gitbash" in
      yes|y|Y)
        log_info "Open https://git-scm.com/downloads"
        command -v powershell.exe && powershell.exe start https://git-scm.com/downloads
        exit 0
        ;;
      *)
        echo "1. Git Bash: https://git-scm.com/downloads"
        echo "2. VirtualBox: https://www.virtualbox.org/wiki/Downloads"
        echo "3. Vagrant: https://developer.hashicorp.com/vagrant/downloads"
        exit 1
        ;;
    esac
  fi
  ...
  if ! command -v VBoxManage &>/dev/null || ! command -v vagrant &>/dev/null; then
    echo "Missing:"
    ! command -v VBoxManage &>/dev/null && echo " - VirtualBox"
    ! command -v vagrant &>/dev/null && echo " - Vagrant"
    echo "Install both before proceeding."
    exit 1
  fi

  if [[ ! -f "$PROJECT_PATH/Vagrantfile" ]]; then
    read -rp "Vagrantfile missing. Is this the correct directory? (yes/no): " yn
    [[ "$yn" =~ ^[Nn] ]] && read -rp "Enter project path: " custom && export PROJECT_PATH="$custom"
  fi

  if [[ -f "$PROJECT_PATH/Vagrantfile" ]]; then
    cd "$PROJECT_PATH"
    vagrant up || {
      read -rp "vagrant up failed. Destroy and retry? (yes/no): " retry
      [[ "$retry" =~ ^[Yy] ]] && vagrant destroy -f && vagrant up || {
        echo "Fix issues and run: vagrant up --provision"
        exit 1
      }
    }
    log_success "Vagrant is ready."
    exit 0
  else
    log_error "No Vagrantfile at $PROJECT_PATH"
    exit 1
  fi
}
